...
export class AuthClient {
    ...
    updateUserInformation(dto: UpdateUserDto): Promise<UserData> {
        let url_ = this.baseUrl + "/api/auth/UpdateUserInformation";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processUpdateUserInformation(_response);
        });
    }

    protected processUpdateUserInformation(response: Response): Promise<UserData> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
    ...
    updatePassword(dto: ChangePasswordRequest): Promise<boolean> {
    ...
    protected processUpdatePassword(response: Response): Promise<boolean> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
...
export class TransactionClient {
    ...
    getAmountOfTransactions(): Promise<number> {
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processGetAmountOfTransactions(_response);
        });
    }

    protected processGetAmountOfTransactions(response: Response): Promise<number> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
